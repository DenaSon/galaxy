<div>
    @if(!Auth::check())
        ...
        <x-modal wire:model="loginModal" class="backdrop-blur-0" box-class="w-96" style="z-index: 1000">
            ...
            <div class="flex items-center justify-center">
                ...
                <x-button spinner="sendVerifySms" icon="o-arrow-right-end-on-rectangle"
                          class="btn-primary w-2/4 mt-3 rounded-lg"
    ...
    @if(Auth::check() && Auth::user()->hasRole('customer'))
        <x-button class="sm:text-xs text-xs" link="{{ route('panel.profile.profileDashboard') }}" icon="o-user-circle"
                  label="پروفایل"/>
    @elseif(Auth::check() && Auth::user()->hasRole('master'))
        <x-button class="sm:text-xs text-xs" link="{{ route('master.dashboard') }}" icon="o-user-circle"
                  label="پنل مدیریت"/>
    @else
        <x-button spinner class="sm:text-xs text-xs" icon="o-user" label="ورود | ثبت‌نام"
                  @click="$wire.loginModal = true"/>
    @endif
