        ...
        <x-form>
            ...
            <x-slot:actions>
                <x-button icon="o-arrow-up-tray" wire:confirm="تصاویر آپلود شوند؟"
                          wire:click="uploadPhotos({{$productId}})" class="btn bg-blue-200"
                          spinner="uploadPhotos"></x-button>
            </x-slot:actions>
        </x-form>
        ...
        @if(!empty($photo_list))
            ...
            @foreach($photo_list as $photo)
                ...
                <div class="relative" wire:key="{{$photo->id}}">
                    <img src="{{ asset($photo->file_path) }}" alt="Image" class="rounded-lg" style="width: 52px;height: 58px" width="52" height="58">
                    <x-button
                        spinner
